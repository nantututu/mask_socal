    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if view_img:
                ...
                fps  = (1./(time_sync()-t1)) 
                print("fps= %.2f"%(fps))
                im0 = cv2.putText(im0, "FPS: %.2f"%(fps), (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 1)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weights/mask_yolov5n-sim.onnx', help='model path(s)')
